...
public class LandlordPostRepositoryImpl implements LandlordPostRepository {
    ...
    @Override
    public List<Landlordpost> getLPosts(double lat, double lng, double radius) {
        Session s = this.factory.getObject().getCurrentSession();
        // Gọi stored procedure bằng SQL Native Query
        String sql = "CALL GetLandlordPostsWithinDistance(:lat, :lng, :distance)";
        Query query = s.createNativeQuery(sql, Landlordpost.class);
